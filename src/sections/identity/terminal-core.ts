...
export type TerminalCommandHandler = (args: string[], ctx: CommandContext) => Promise<ExecResult>
export type TerminalCommandOverrides = Record<string, TerminalCommandHandler | undefined>
...
const BASH_COMMANDS = getCommandNames().filter((name): name is CommandName => !EXCLUDED_COMMANDS.has(name))
const NETWORK_COMMANDS = getNetworkCommandNames()
const COMPLETE_COMMANDS = [...new Set([...BASH_COMMANDS, ...NETWORK_COMMANDS, ...CUSTOM_COMMAND_NAMES])]
...
export async function executeTerminalCommand(session: TerminalSession, value: string): Promise<TerminalExecutionResult | undefined> {
	...
	try {
		...
		syncBashState(session)
		const result = await session.bash.exec(input, { cwd: session.cwd, env: session.env })
...
export function normalizeTerminalFiles(rawFiles: Record<string, string>, prefix = '/fs') {
	return Object.fromEntries(Object.entries(rawFiles).map(([path, content]) => [path.startsWith(prefix) ? path.slice(prefix.length) : path, content]))
}

function createCustomCommands(history: string[], signals: TerminalSignals, options: TerminalSessionOptions) {
	const loadGitLog = options.loadGitLog ?? loadDefaultGitLog
	const command = (name: string, execute: TerminalCommandHandler) => defineCommand(name, options.commandOverrides?.[name] ?? execute)

	return [
		command('portfolio', async (_args, ctx) => readVirtualFile(ctx, '/home/sheraff/about.txt')),
		command('mask', async (_args, ctx) => readVirtualFile(ctx, '/home/sheraff/documents/mask.txt')),
		command('ll', async (args, ctx) => execFromContext(ctx, ['ls', '-la', ...args].map(quoteArg).join(' '))),
		command('clear', async () => {
		...
		command('wget', async () => stdoutLine('wget: network access is disabled in this tiny universe')),
		command('ssh', async () => stdoutLine('ssh: network access is disabled in this tiny universe')),
...
export async function resolveGit(args: string[], loadGitLog = loadDefaultGitLog) {
	if (args.length === 0) {
